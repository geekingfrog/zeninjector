...
Container.prototype.resolve = function(name) {
  ...
  if(module.state === 'resolved') { return module.exported; }

  if(module.state === 'resolving') {
    return Promise.reject(new ModuleError('Circular dependency detected with '+name));
  }

  try {
    module.dependencies.forEach(function(dep) {
      if( !this._modules[dep] ) {
        console.log('%s not found', dep);
